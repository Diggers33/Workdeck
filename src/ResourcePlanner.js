...
const ResourcePlanner = () => {
  ...
  useEffect(() => {
    ...
    const initializeTeamData = () => {
      console.log('Initializing team data with static members:', staticTeamMembers.length);
      ...
      console.log('Setting unified data:', unifiedData);
  ...
  console.log('Rendering with teamMembers:', teamMembers.length, 'members');
  
  // Always show content, don't wait for data
  if (!teamMembers || teamMembers.length === 0) {
    console.log('No team members found, using static data');
    return (
      <div className="bg-gray-50 min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="text-lg font-semibold text-gray-900">Loading Resource Planner...</div>
          <div className="text-sm text-gray-600 mt-2">Initializing team data ({staticTeamMembers.length} members)</div>
        </div>
      </div>
    );
  }
  ...
  const syncTaskFromSpreadsheet = (memberId, projectId, monthIndex, hours) => {
    console.log('Syncing from spreadsheet:', { memberId, projectId, monthIndex, hours });
          ...
          return {
            ...
            tasks: member.tasks.map(task => {
              if (task.projectId === projectId) {
                ...
                console.log('Updated monthly hours:', newMonthlyHours);
  ...
  const syncSpreadsheetFromTask = (memberId, taskId, updates) => {
    console.log('Syncing from task:', { memberId, taskId, updates });
            ...
            tasks: member.tasks.map(task => {
              if (task.id === taskId) {
                ...
                if (updates.targetHoursPerWeek !== undefined) {
                  const newMonthlyHours = task.monthlyHours ? task.monthlyHours.map(hours => 
                    hours > 0 ? updates.targetHoursPerWeek : 0
                  ) : Array(12).fill(updates.targetHoursPerWeek || 0);
                  updatedTask.monthlyHours = newMonthlyHours;
                  console.log('Updated task monthly hours:', newMonthlyHours);
                }
  ...
  const handleProjectCellEdit = (memberId, projectId, columnIndex, value) => {
    ...
    const clampedValue = Math.min(numValue, 300);
    
    // Convert monthly hours to weekly hours for storage
    let weeklyHours = clampedValue / 4.33; // Convert from monthly to weekly
    
    console.log('Storing weekly hours:', weeklyHours);
        ...
        {showSpreadsheetView ? (
          ...
          <div className="mb-8">
            <div className="mb-4 p-3 bg-gray-800 text-white rounded">
              ...
              <p className="text-sm text-gray-300">{teamMembers.length} team members</p>
            </div>
                      ...
                      {selectedView === 'year' ? (
                        (() => {
                          ...
                          for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
                            const monthHours = member.tasks.reduce((sum, task) => {
                              return sum + (task.monthlyHours ? (task.monthlyHours[monthIndex] || 0) * 4.33 : 0);
                            }, 0);
                            hours += monthHours;
                          }
                          
                          const colorClass = hours > 1600 ? 'bg-red-500 text-white' : 
                                            hours > 1200 ? 'bg-orange-500 text-white' : 
                                            hours > 0 ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-400';
                          
                          return (
                            <div className={`h-6 rounded flex items-center justify-center text-xs font-semibold ${colorClass}`}>
                              {hours === 0 ? '—' : `${Math.round(hours)}h`}
                            </div>
                          );
                        })()
                        ...
                        Array.from({ length: 3 }, (_, periodIndex) => {
                          // Calculate hours for each month in quarter (Apr, May, Jun)
                          const monthIndex = 3 + periodIndex; // Apr=3, May=4, Jun=5
                          const hours = member.tasks.reduce((sum, task) => {
                            return sum + (task.monthlyHours ? (task.monthlyHours[monthIndex] || 0) * 4.33 : 0);
                          }, 0);
                          
                          const colorClass = hours > 173 ? 'bg-red-500 text-white' : 
                                            hours > 130 ? 'bg-orange-500 text-white' : 
                                            hours > 0 ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-400';
                          
                          const isCurrentPeriod = periodIndex === 2; // June is current
                          
                          return (
                            <div key={periodIndex} className={`h-6 rounded flex items-center justify-center text-xs font-semibold ${colorClass} ${isCurrentPeriod ? 'ring-1 ring-blue-400' : ''}`}>
                              {hours === 0 ? '—' : `${Math.round(hours)}h`}
                            </div>
                          );
                        })
                        ...
                        Array.from({ length: 4 }, (_, periodIndex) => {
                          // Calculate hours for each week in June
                          const currentMonth = 5; // June = 5
                          const weeklyHours = member.tasks.reduce((sum, task) => {
                            return sum + (task.monthlyHours ? (task.monthlyHours[currentMonth] || 0) : 0);
                          }, 0);
                          
                          const colorClass = weeklyHours > 40 ? 'bg-red-500 text-white' : 
                                            weeklyHours > 30 ? 'bg-orange-500 text-white' : 
                                            weeklyHours > 0 ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-400';
                          
                          const isCurrentPeriod = periodIndex === 2; // Week 3 is current
                        ...
                        Array.from({ length: 9 }, (_, periodIndex) => {
                          ...
                          const dailyHours = weeklyHours / 5; // Distribute across 5 working days
                          
                          const colorClass = dailyHours > 8 ? 'bg-red-500 text-white' : 
                                            dailyHours > 6 ? 'bg-orange-500 text-white' : 
                                            dailyHours > 0 ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-400';
                          
                          const isCurrentPeriod = periodIndex === 0; // Today
