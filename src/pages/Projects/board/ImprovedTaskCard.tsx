...
export function ImprovedTaskCard({
  ...
  const isOverdue = task.dueDate ? (() => {
    ...
    // Use November 24, 2024 as "today" for consistent demo
    const today = new Date(2024, 10, 24); // Month 10 = November (0-indexed)
  ...
  const daysOverdue = isOverdue && task.dueDate ? 
    Math.floor((new Date(2024, 10, 24).getTime() - new Date(task.dueDate).getTime()) / (1000 * 60 * 60 * 24)) : 0;

  const isDueToday = task.dueDate && (() => {
    const parts = task.dueDate.split('-');
    const dueDate = new Date(parseInt(parts[0]), parseInt(parts[1]) - 1, parseInt(parts[2]));
    const today = new Date(2024, 10, 24); // November 24, 2024
    dueDate.setHours(0, 0, 0, 0);
    today.setHours(0, 0, 0, 0);
    return dueDate.getTime() === today.getTime();
  })();
